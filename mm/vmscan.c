...
/* MemBo */
static bool prepare_membo_manager_sleep(void)
{
    return true;
}

static void membo_manager_try_to_sleep(pg_data_t *pgdat)
{
    long remaining = 0;
	DEFINE_WAIT(wait);
	...
	prepare_to_wait(&pgdat->membo_manager_wait, &wait, TASK_INTERRUPTIBLE);
    if (prepare_membo_manager_sleep()) {
        remaining = schedule_timeout(HZ/10);

        finish_wait(&pgdat->membo_manager_wait, &wait);
		prepare_to_wait(&pgdat->membo_manager_wait, &wait, TASK_INTERRUPTIBLE);
    }
    if (!remaining &&
	    prepare_membo_manager_sleep()) {
        if (!kthread_should_stop()) {
            schedule();
        }
    }
...
void wakeup_membo_manager(struct zone *zone, int order)
{
    ...
    if (!waitqueue_active(&pgdat->membo_manager_wait))
		return;

    atomic_set(&pgdat->membo_mcounter, 0);
    wake_up_interruptible(&pgdat->membo_manager_wait);
}

int (*membo_request_mram_borrowing)(int nid);
EXPORT_SYMBOL(membo_request_mram_borrowing);
...
membo_manager_try_to_sleep:
        membo_manager_try_to_sleep(pgdat);
        ...
        if (membo_request_mram_borrowing)
            membo_ret = membo_request_mram_borrowing(pgdat->node_id);

        /* If we fail to borrow MRAM, disable membo_manager */
        if (membo_ret)
            atomic_set(&pgdat->membo_disabled, 1);
    }
    return 0;
}
void membo_init_node(int nid)
{
    ...
    init_waitqueue_head(&pgdat->membo_manager_wait);
    init_waitqueue_head(&pgdat->membo_reclaimer_wait);
}
EXPORT_SYMBOL(membo_init_node);

void membo_manager_run(int nid)
{
    pg_data_t *pgdat = NODE_DATA(nid);
    if (pgdat->membo_manager)
		return;
    pgdat->membo_manager = kthread_run(membo_manager, pgdat, "membo_manager%d", nid);
}
EXPORT_SYMBOL(membo_manager_run);
...
void wakeup_membo_reclaimer(int nid)
{
    ...
    if (!waitqueue_active(&pgdat->membo_reclaimer_wait))
		return;

    wake_up_interruptible(&pgdat->membo_reclaimer_wait);
}
EXPORT_SYMBOL(wakeup_membo_reclaimer);

static void membo_reclaimer_try_to_sleep(pg_data_t *pgdat)
{
    long remaining = 0;
    DEFINE_WAIT(wait);
...
static void membo_reclaimer_try_to_reclaim(pg_data_t *pgdat)
{
    unsigned long mark = -1;
    struct zone *zone;
    ...
    mark = low_wmark_pages(zone) + (1 << 30)/PAGE_SIZE;

    if (zone_watermark_ok_safe(zone, 0, mark, MAX_NR_ZONES)) {
        if (atomic_inc_return(&pgdat->membo_rcounter_n) == 10) {
            atomic_set(&pgdat->membo_rcounter_n, 0);
            goto do_reclamation;
        }
    }
    ...
    mark = low_wmark_pages(zone) + (1600 << 20)/PAGE_SIZE;

    if (zone_watermark_ok_safe(zone, 0, mark, MAX_NR_ZONES)) {
        if (atomic_inc_return(&pgdat->membo_rcounter_d) == 10) {
            atomic_set(&pgdat->membo_rcounter_d, 0);
            goto do_reclamation;
        }
    }
...
do_reclamation:
    if (membo_request_mram_reclamation)
        membo_request_mram_reclamation(pgdat->node_id);
}

static int membo_reclaimer(void *p)
{
    pg_data_t *pgdat = (pg_data_t *)p;
...
membo_reclaimer_try_to_sleep:
        membo_reclaimer_try_to_sleep(pgdat);
...
{
    pg_data_t *pgdat = NODE_DATA(nid);
    if (pgdat->membo_reclaimer)
		return;
    pgdat->membo_reclaimer = kthread_run(membo_reclaimer, pgdat, "membo_reclaimer%d", nid);
}
EXPORT_SYMBOL(membo_reclaimer_run);
